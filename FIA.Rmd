...
So I want to plot time by species. But this is going to be tricky because the dataset has years with speceis repeated. I want a cumulative quantity for each species for each year. I'm just going to sum basal area (BA) for now. But I can change this if there's a different measure we want to look at. 
...
So a problem that really sticks out at this point is that there are a lot of speceis. It may be worth just looking at species I build PFT's for -major/more dominant over story speceis. 
...
```{r}
#this for loop will only work if year is in the first column and basal area is in the third column. 
vtTrees8$total.BA <- 0


for(i in seq_along(vtTrees8$total.BA)) {
  sum(vtTrees8[which(vtTrees8[,1]==vtTrees),3])
}

for(i in seq_along(vtTrees8$INVYR)) {
  sum(filter(vtTrees7, INVYR %in% vtTrees7[1,]), 3) #filter for rows whose value for INVYR are equal to a value in the first row. 
}

filter(vtTrees7, INVYR %in% vtTrees7[1,])#filter for rows whose value for INVYR are equal to a value in the first row. 
